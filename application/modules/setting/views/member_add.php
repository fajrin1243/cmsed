					...
					<div class="row"> 
						<div class="col-lg-12"> 
							<form class="form-horizontal" role="form" method="post" action="<?php echo base_url() ?>index.php/<?php echo getModule() ?>/<?php echo getController() ?>/save">
								...
								<?php echo input_text_group('usernameMember','Username',(@$getMember[0]['usernameUser']) ? @$getMember[0]['usernameUser'] : "",'Username User','required') ?>
								<?php echo input_text_group('nameUser','Nama',(@$getMember[0]['nameUser']) ? @$getMember[0]['nameUser'] : "",'Nama User','required') ?>
								<div class="form-group">
									<label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">Password</label>
									<div class="col-sm-6">
										<input type="text" name="passwordMember" class="form-control" placeholder="Password Member" value="<?php echo ($getMember) ? $getMember[0]['passwordUser'] : "" ?>">
									</div>
								</div>
								<?php echo input_email_group('emailUser','Email',(@$getMember[0]['emailUser']) ? @$getMember[0]['emailUser'] : "",'Email','required') ?>
								<?php echo select_join_group('namaRole','idRole,namaRole','master_user_role','idRole','roleMember','Role','',($getMember) ? $getMember[0]['roleUser'] : "",'required') ?>
								<div class="form-group">
									<label for="inputEmail1" class="col-lg-2 col-sm-2 control-label">No. Ponsel</label>
									<div class="col-sm-6">
										<input type="text" name="hpMember" class="form-control" placeholder="No. Telepon" value="<?php echo ($getMember) ? $getMember[0]['phone'] : "" ?>">
									</div>
								</div>
